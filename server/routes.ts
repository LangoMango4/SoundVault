...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/private-messages', isAuthenticated, async (req, res, next) => {
    try {
      if (!req.user) {
        return res.status(401).json({ message: "Unauthorized" });
      }
      
      const messages = await storage.getPrivateMessagesForUser(req.user.id);
      
      // Group messages by conversation partners and add user details
      const conversations = new Map();
      
      for (const message of messages) {
        const otherUserId = message.senderId === req.user.id ? message.recipientId : message.senderId;
        
        // Get or create the conversation entry
        if (!conversations.has(otherUserId)) {
          const otherUser = await storage.getUser(otherUserId);
          if (!otherUser) continue; // Skip if the other user doesn't exist
        ...
        // Count unread messages (if current user is the recipient and hasn't read it)
      ...
      conversationsArray.forEach(conv => {
        conv.messages.sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
      });
      
      // Sort conversations by latest message
      conversationsArray.sort((a, b) => {
        const aLatest = a.messages[0]?.timestamp || new Date(0);
        const bLatest = b.messages[0]?.timestamp || new Date(0);
  ...
  app.get('/api/private-messages/:userId', isAuthenticated, async (req, res, next) => {
    try {
      if (!req.user) {
        return res.status(401).json({ message: "Unauthorized" });
      }
      ...
      for (const message of messages) {
        if (message.recipientId === req.user.id && !message.isRead) {
          await storage.markPrivateMessageAsRead(message.id);
        }
      }
  ...
  app.post('/api/private-messages', isAuthenticated, async (req, res, next) => {
    try {
      if (!req.user) {
        return res.status(401).json({ message: "Unauthorized" });
      }
      ...
      const validation = insertPrivateMessageSchema.safeParse(messageData);
      if (!validation.success) {
      ...
      const { password: _, ...senderInfo } = req.user;
  ...
  app.post('/api/private-messages/:id/read', isAuthenticated, async (req, res, next) => {
    try {
      if (!req.user) {
        return res.status(401).json({ message: "Unauthorized" });
      }
      ...
      // Mark as read
      const updatedMessage = await storage.markPrivateMessageAsRead(messageId);
      if (!updatedMessage) {
  ...
  // Delete a message
  app.delete('/api/private-messages/:id', isAuthenticated, async (req, res, next) => {
    try {
      if (!req.user) {
        return res.status(401).json({ message: "Unauthorized" });
      }
      ...
      if (message.senderId !== req.user.id && req.user.role !== 'admin') {
        return res.status(403).json({ message: "You can only delete messages you've sent" });
      }
      
      // Delete the message
      const success = await storage.deletePrivateMessage(messageId);
      if (!success) {
